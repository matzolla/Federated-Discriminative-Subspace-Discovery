...
def main():
    ...
    device= args.device
    model = nn.Linear(TOP_K, num_classes).to(device)
    global_classifier= LoRALinear(model,r=32,alpha=64).to(device) #we set alpha = 2*r like in the paper. we also set r=45 and 32
    ...
    for r in tqdm(range(args.num_rounds)):
        ...
        for emb, labls in zip(client_emb,client_labels):
            state, n = local_train_classifier(emb, labls, global_state, device, num_classes,method='lora')
        ...
        global_state = Fed_avg_aggregate_classifier(local_states, client_sizes,bits=None) 
...
if __name__ == "__main__":
    parser= get_parser()
    args = parser.parse_args()
    log_path=f"logs/{args.num_classes}_Flocora_r_64_{args.log_file}"
